/*
 * etapa6.cpp — a etapa 5 mais isolamento do sistema operacional.
 *
 * A etapa 5 mostrou dois problemas distintos:
 *
 *   RESOLUCAO   quatro dos seis benchmarks medem poucas dezenas de ticks, e a
 *               regua de 18,52 ns nao os resolve. Isso e' assunto da etapa 8.
 *
 *   REPRODUTIBILIDADE   o matmult mediu 4149 ticks numa rodada e 2596 noutra,
 *               razao 1,598, que e' exatamente 2,4 GHz / 1,5 GHz. Cada lote e'
 *               internamente consistente e os lotes discordam entre si. Este e'
 *               o assunto DESTA etapa.
 *
 * Quatro fontes de perturbacao, atacadas uma a uma:
 *
 *   1. o escalonador move sua thread entre nucleos       -> sched_setaffinity
 *   2. o escalonador tira sua thread da CPU              -> SCHED_FIFO
 *   3. faltas de pagina dentro da regiao medida          -> mlockall
 *   4. o clock do nucleo muda no meio do lote            -> governor
 *
 * As tres primeiras o programa resolve sozinho (com sudo). A quarta ele apenas
 * MEDE E DENUNCIA, porque mexer no governor e' uma mudanca global da maquina e
 * essa decisao e' sua, nao do benchmark.
 *
 * Um binario por benchmark:  bin6/bsort100, bin6/crc, ...
 *
 * Uso:  ./bin6/<nome> [-n EXEC] [-w AQUECIMENTOS] [-c NUCLEO]
 *
 * Rode com e sem sudo e compare. Sem sudo o programa avisa o que nao conseguiu.
 */
...
{
	fprintf(stderr,
		"uso: %s [-n EXEC] [-w AQUECIMENTOS] [-c NUCLEO]\n"
...
int main(int argc, char **argv)
{
	...
	printf("etapa6 — benchmark \"%s\"\n", BENCH_NAME);
	printf("  %d execucoes medidas, %d de aquecimento\n", n, aquecimentos);
	printf("  CNTFRQ_EL0 = %.3f MHz (%.2f ns/tick)\n\n",
	       f_timer / 1e6, ns_por_tick);

	/* Bloco de ambiente: sem isto, um numero colado num relatorio nao diz sob
	 * que condicoes foi obtido, e nao e' reproduzivel nem por voce mesma. */
	printf("AMBIENTE\n");
	printf("  afinidade   nucleo %d          %s\n", nucleo,
	       ok_afinidade ? "OK" : strerror(erro_afinidade));
	printf("  nucleo real onde rodou: %d      %s\n", nucleo_real,
	       nucleo_real == nucleo ? "" : "<<< NAO e' o pedido");
	printf("  SCHED_FIFO  prioridade 80     %s\n",
	       ok_fifo ? "OK" : strerror(erro_fifo));
	printf("  mlockall    CURRENT|FUTURE    %s\n",
	       ok_mlock ? "OK" : strerror(erro_mlock));
	printf("  governor    %-16s %s\n", governor,
	       strcmp(governor, "performance") == 0 ? "OK" : "<<< nao e' performance");
	printf("  clock       %.0f MHz antes, %.0f MHz depois   %s\n",
	       f_antes / 1e6, f_depois / 1e6,
	       (f_antes && f_antes == f_depois) ? "estavel" : "<<< MUDOU");
